...
{
  Serial.print("STATUS = 0x0"); Serial.print(status,HEX);
  Serial.print(" RX_DR="); Serial.print((status & _BV(RX_DR))?1:0);
  Serial.print(" TX_DS="); Serial.print((status & _BV(TX_DS))?1:0);
  Serial.print(" MAX_RT=");Serial.print((status & _BV(MAX_RT))?1:0);
  Serial.print(" RX_P_NO=");Serial.print((status >> RX_P_NO) & 0b111);
  Serial.print(" TX_FULL=");Serial.println((status & _BV(TX_FULL))?1:0);
}
...
void RF24::print_byte_register(const char* name, uint8_t reg, uint8_t qty)
{
  ...
  while (qty--) {
    if(read_register(reg) > 0x0F) {
      Serial.print("0x");
      Serial.print(read_register(reg++), HEX);
      Serial.print(" ");
    }
    else {
      Serial.print("0x0");
...
void RF24::print_address_register(const char* name, uint8_t reg, uint8_t qty)
{
  ...
  while (qty--)
  {
    ...
    while( --bufptr >= buffer ) {
      Serial.print("0x");Serial.print(*bufptr, HEX);Serial.print(" ");
    }
  }
...
void RF24::printDetails(void)
{
  ...
  Serial.print("Data Rate = ");Serial.println(pgm_read_word(&rf24_datarate_e_str_P[getDataRate()]));
  Serial.print("Model = ");Serial.println(pgm_read_word(&rf24_model_e_str_P[isPVariant()]));
  Serial.print("CRC Length = ");Serial.println(pgm_read_word(&rf24_crclength_e_str_P[getCRCLength()]));
  Serial.print("PA Power = ");Serial.println(pgm_read_word(&rf24_pa_dbm_e_str_P[getPALevel()]));
